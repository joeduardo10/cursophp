<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Desafio 10</title><link rel="stylesheet" href="style.css">
    <link rel="shortcut icon" href="Elephant-icon.png" type="image/x-icon">
</head>
<body>
    <?php 
    $atual = date("Y");
    $nasc = $_GET['ano_nasc']?? '2000';
    $ano = $_GET['ano']?? $atual;
    ?>
    <main>
        <h1>Máquina do Tempo</h1>
        <form action="<?=$_SERVER['PHP_SELF']?>" method="get">
        <label for="ano_nasc">Em que ano você nasceu?</label>
        <input type="number" name="ano_nasc" id="ano_nasc" min="1900" max="<?=$atual-1?>" value="<?=$nasc?>"> 
        <label for="ano">Quer Saber a sua idade em que ano?(atualmete estamos em:<strong><?=$atual?></strong>)</label>
        <input type="number" name="ano" id="ano" min="1900" value="<?=$ano?>">
        <input type="submit" value="Qual será a minha idade?">
        </form>
    </main>
    <section>
        <?php 
        $idade = $ano - $nasc;
        
        ?>
        <h2>Resultado</h2>
        <p>quem nasceu em [<?=$nasc?>]vai ter <strong>[<?=$idade?>] anos </strong>em [<?=$ano?>]</p>
    </section>
</body>
</html>